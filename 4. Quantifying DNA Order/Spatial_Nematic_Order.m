...
%bins are cubes with edge length = chromosome long axis length / nBins
%analyze largest 75 scaffolds only as these are most likely completely assembled chromosomes
all_alignment_4bins = NOP(4, 75); %nBins, num_chroms
all_alignment_6bins = NOP(6, 75); %nBins, num_chroms
...
function all_alignment = NOP(nBins, num_chroms) %if error message try reducing nBins
all_alignment = zeros(num_chroms,6);
for k = 1:1:6
    k
    for i = 1:1:num_chroms
        ...
        % figure
        % hold on
        % plot3(xx,yy,zz,'k',LineWidth=1.2)
        ...
        % patch(xc4(idx),yc4(idx),zc4(idx), 'r', 'facealpha', 0.2);
        % patch(xc6(idx)-35,yc6(idx),zc6(idx), 'r', 'facealpha', 0.2);
        % axis equal
        % view(0,0)
        % set(gca,'xtick',[])
        % set(gca,'xticklabel',[])
