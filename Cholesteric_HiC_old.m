...
for i=1:1:numloops
    ...
    %reject walks with any steps of length spacing*1.5 or greater
    j=1;
    while j<=size(loops{i},1)-1
        while sqrt((loops{i}{j,1})-loops{i}{j+1,1}).^2 + (loops{i}{j,2}-loops{i}{j+1,2}).^2 + (loops{i}{j,3}-loops{i}{j+1,3}).^2 > spacing*1.5
            if mod(count,2)==1
                loops{i}=num2cell(constrained_self_avoiding_RW_3D(disc_starts(count,:),disc_starts(count+1,:),round(exponential_loops(i)),spacing));
                j=1;
            else
