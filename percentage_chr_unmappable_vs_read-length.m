...
[header_kawagutii,sequence_kawagutii] = fastaread('Fugacium_kawagutii_V3_genome_scaffold.FINAL.fa');
Density_kawagutii = cell(size(header_kawagutii,2),1);
NTStruct_kawagutii = cell(size(header_kawagutii,2),1);
unmappable_kawagutii = cell(size(header_kawagutii,2),1);
percent_N_kawagutii = zeros(size(header_kawagutii,2),1);

% for i = 1:size(header_kawagutii,2)
%     Density_kawagutii{i} = ntdensity(sequence_kawagutii{i},'Window',5000);
% end

for i = 1:size(header_kawagutii,2)
    NTStruct_kawagutii{i} = basecount(sequence_kawagutii{i},'Ambiguous', 'individual');
    percent_N_kawagutii(i) = NTStruct_kawagutii{i}.N./length(sequence_kawagutii{i})*100;
end
...
for j = 1:1:100
    ...
    for i = 1:1:size(count,2)
        nhist(i)=nhist(i).*round(count(i))./length(sequence_kawagutii{j}).*100;
    end

    unmappable_kawagutii{j} = nhist;
    j
end
...
hold on
for i = 1:1:100
    plot(count,unmappable_kawagutii{i}', 'Color', [0 0.4470 0.7410])
end
%ChIP-seq read length: ~100 bp
%DiMeLo-seq read length: ~100 000 bp
xline(100,'-r',{'Short read sequencing','e.g. ChIP-seq & ATAC-seq'}, 'FontSize', 12);
...
ylim([0 10])
set(gca, 'XScale', 'log')
title('Symbiodinium kawagutii', 'FontSize', 18)